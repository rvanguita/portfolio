...
def test_config():
    ...
    assert "theme: jekyll-theme-cayman" not in content, "O tema legado 'cayman' ainda está ativo!"
...
def test_css_integrity():
    ...
    assert open_braces == close_braces, f"Chaves desbalanceadas no CSS: {open_braces} abertas vs {close_braces} fechadas"
    
    # Verifica variáveis principais
    assert ":root" in content, "Bloco :root ausente no CSS"
    assert '[data-theme="dark"]' in content, "Bloco de modo escuro [data-theme='dark'] ausente no CSS"
    assert "--bg-page" in content, "Variável --bg-page ausente"
    assert "--accent" in content, "Variável --accent ausente"
    
    log_pass(f"custom.css válido com {open_braces} blocos de estilo balanceados e modo escuro configurado.")
...
def test_index_html_and_local_assets():
    ...
    for link in parser.local_links:
        ...
        if clean_link.startswith(("/certificates/", "/assets/")):
            ...
            assert target_path.exists(), f"Arquivo referenciado não existe no disco: {target_path}"
            checked_files += 1
            
    log_pass(f"Todos os {checked_files} arquivos locais (imagens, CSS e PDFs de certificados) foram validados com sucesso!")
...
def test_readme_certificates():
    ...
    for link in cert_links:
        ...
        assert target.exists(), f"Certificado listado no README.md não existe: {decoded_link}"
        
    log_pass(f"Todos os {len(cert_links)} links de certificados no README.md foram verificados e existem no disco.")
...
def main():
    ...
    print("\n" + "=" * 65)
    if failed == 0:
        print("\033[92m🎉 SUCESSO: Todos os testes passaram! O site está seguro para deploy.\033[0m")
        print("=" * 65)
        sys.exit(0)
    else:
        print(f"\033[91m💥 FALHA: {failed} teste(s) falharam! Corrija os erros antes do deploy.\033[0m")
        print("=" * 65)
        sys.exit(1)
